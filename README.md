# Fractie Franssen Website

Deze website is volledig geoptimaliseerd voor desktop, tablet en mobiele apparaten.


## Belangrijke Features


### Desktop Features
- Volledig functionele navigatie met hover effecten
- Typing animatie in hero sectie
- Smooth scroll naar secties
- Modal popups voor standpunten met gedetailleerde informatie

### Breakpoints
- **Desktop**: > 1024px
- **Tablet**: 768px - 1024px
- **Mobile**: < 768px
- **Small Mobile**: < 480px

## Browser Ondersteuning

- Chrome/Edge (laatste 2 versies)
- Firefox (laatste 2 versies)
- Safari (laatste 2 versies)
- Mobile browsers (iOS Safari, Chrome Mobile)

## Aanpassingen Maken

### Kleuren wijzigen
Bewerk `css/variables.css` om de hoofdkleuren aan te passen.

### Tekst wijzigen
Bewerk `index.html` voor alle statische tekst.

### Modal content wijzigen
Bewerk `js/modals.js` - zoek naar `modalData` object.

### Animaties aanpassen
Bewerk `js/typing.js` voor de typing animatie of `js/animations.js` voor scroll effecten.
